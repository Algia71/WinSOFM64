...
Applicazione della quantizzazione vettoriale operata dalla Self Organizing Features Map (SOFM) di Kohonen sulla distribuzione omogenea di punti nel piano.
...
Il programma, scritto in linguaggio C nel 1996,  fu realizzato originariamente con QuickC For Windows, sistema opeativo target Windows 3.0. Questa è una ricompilazione a 64 bit realizzata con Visual Studio Community 2022. Nella sua trasposizione da 16 a 64 bit, il programma conserva il modello cooperativo di multi tasking, originariamente necessario per permettere alla GUI di continuare a rispondere durante la fase di simulazione. E' interessante notare come non sia stato necessario apportare alcuna modifica al codice sorgente C al fine di effettuare il porting da 16 a 64 bit, al netto ovviamente dei file di progetto.

_Technical details. The program, written in C language in 1996, was originally developed with QuickC For Windows; the target operating system was Windows 3.0. This is a 64-bit recompilation made with Visual Studio Community 2022. In its port from 16 to 64 bits, the program retains the cooperative multitasking model, originally necessary to allow the GUI to remain responsive during the simulation phase. Interestingly, no changes were needed to the C source code to port from 16 to 64 bit, apart from the project files, of course._
